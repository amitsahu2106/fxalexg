...
scores confluence, calls Claude for analysis, sends Telegram alerts.
"""
...
OANDA_BASE_URL     = "https://api-fxpractice.oanda.com"   # Demo account URL
...
MIN_GRADE_TO_ALERT = ['A+', 'B+'] # Only alert for these grades
ALERT_COOLDOWN_MIN = 60           # Don't re-alert same pair for 60 mins
...
def pip_size(pair):
    ...
    if 'XAU' in pair:   return 0.10   # $0.10 per point for Gold
    return 0.0001
...
# ─────────────────────────────────────────────
# AOI DETECTOR — max 60 pips wide, min 2 touches
# ─────────────────────────────────────────────
...
def detect_hs(candles, highs, lows):
    ...
    # Return best (prefer complete patterns)
...
def score(trends, aoi_zone, hs, ema_sig):
    ...
    if aoi_zone:
        pts += 2
        reasons.append(f"✅ At AOI — {aoi_zone['size_pips']}p wide, {aoi_zone['touches']} touches")
    else:
    ...
    if ema_sig:
        pts += 1
        reasons.append(f"✅ H1 price {'above' if ema_sig == 'ABOVE' else 'below'} 21 EMA")
...
def ask_gemini(pair, data):
    ...
    hs_txt = (
        ...
        f"Neckline: {data['hs']['neckline']} | {'COMPLETE' if data['hs']['complete'] else 'Forming'}"
...
AOI ZONE   : {aoi_txt}
21 EMA (H1): {data['ema']} → Price is {data['ema_sig']}
H&S PATTERN: {hs_txt}
...
# ─────────────────────────────────────────────
_last_alert = {}   # {pair: timestamp}

def can_alert(pair):
    now = time.time()
    last = _last_alert.get(pair, 0)
...
def analyse(pair):
    ...
    for tf in TIMEFRAMES:
        ...
        if tf in ('H4', 'H1'):
            ...
            if hs and (best_hs is None or (hs['complete'] and not best_hs['complete'])):
                best_hs = hs

        if tf == 'H1':
            h1_ema = ema21(candles)
            if h1_ema:
    ...
    msg = f"""{emoji} <b>FXAlexG Alert</b>
<b>{pair.replace('_','/')}  [{grade}]  Score: {pts}/10</b>
...
# ─────────────────────────────────────────────
# MAIN — Single scan run (triggered by GitHub Actions)
# ─────────────────────────────────────────────
def main():
    print("=" * 55)
